...
function loadJoinedGrps() {
   var grp, grpElement, pane = document.getElementById('joinedGrpsSection');
   pane.innerHTML = '';
   pane.innerHTML += '<span>Groups</span>';
   for (let i of curUser.groups) {
      grp = groupList[i];
      grpElement = '<div class="setting-box" onclick="activeGroup=groupList[\''+grp.id+'\'],forward(groupActivity)"><img src="'+grp.photo+'" style="border-radius: 50%"><span id="grpInfoEmail">'+grp.gname+'</span></div>';
      pane.innerHTML += grpElement;
   }
   pane.innerHTML += '<span>My groups</span>';
   for (let i of curUser.ownGroups) {
      grp = groupList[i];
      grpElement = '<div class="setting-box" onclick="activeGroup=groupList[\''+grp.id+'\'],forward(myGroupActivity)"><img src="'+grp.photo+'" style="border-radius: 50%"><span id="grpInfoEmail">'+grp.gname+'</span></div>';
      pane.innerHTML += grpElement;
   }
}
...
function comment() {
   ...
   var comment = {
      ...
      time: months[time.getMonth()].slice(0,3) +" "+ time.getDate() +" "+ time.getHours() +":"+time.getMinutes()
   }
   commentField.value = '';
...
function send(content = []) {
   ...
   var message = {
      ...
      icon: curUser.photo,
      content:content,
      time: months[time.getMonth()].slice(0,3) +" "+ time.getDate() +" "+ time.getHours() + ":" +time.getMinutes(), likers: [], dislikers: [], comments: []
   }
...
function groupActivity(load = true) {
   ...
   document.getElementById('grpInfoSection').innerHTML = '';
   document.getElementById('grpInfoSection').innerHTML += '<span>Members</span>';
   for (let i of groupList[activeGroup.id].members) {
      member = users[i];
      memberElement = '<div class="setting-box"><img src="'+member.photo+'" style="border-radius: 50%"><span id="grpInfoEmail">'+member.fname+" "+member.lname+'</span></div>';
      document.getElementById('grpInfoSection').innerHTML += memberElement;
   }
...
function leaveGroup(){
   ...
   showHideSideBar();
   loadJoinedGrps();
   updateGroupView(activeGroup.id);
   back();
}
...
function myGroupLoader(){
   ...
   document.getElementById("myGroupMembers").innerHTML = activeGroup.members.length + " members";
...
function myGroupActivity(load = false) {
   ...
   var member, memberElement, pane = document.getElementById('myGrpInfoSection');
   pane.innerHTML = '';
   pane.innerHTML += '<span>Members</span>';
   for (let i of groupList[activeGroup.id].members) {
      member = users[i];
      memberElement = '<div class="setting-box"><img src="'+member.photo+'" style="border-radius: 50%"><span id="grpInfoEmail">'+member.fname+" "+member.lname+'</span></div>';
      pane.innerHTML += memberElement;
   }
...
function updateGroupView(groupId, chat = 'searchWrapper'){
   var group = groupList[groupId];
   var action, label, status;
      ...
      modifyElement('join'+chat+group.id, "join-group " + status, label, null, action)

class groupPreviewElement {
	construct(group, chat) {
		...
		if (curUser.groups.indexOf(group.id) == -1) {
			...
			this.icon = makeElement("img", null, null, group.photo,'activeGroup = groupList[\"' + group.id + '\"], forward(groupActivity)' );
			if (curUser.ownGroups.indexOf(group.id) != -1) {
				this.joinStatus = '';
				this.label = "EDIT";
				this.action = 'activeGroup = groupList[\"' + group.id + '\"], forward(myGroupActivity)';
				this.icon = makeElement("img", null, null, group.photo,'activeGroup = groupList[\"' + group.id + '\"], forward(myGroupActivity)' );

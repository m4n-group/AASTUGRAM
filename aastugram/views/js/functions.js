...
function comment() {
   ...
   var commenter = curUser.fname;
   var groupid = activeGroup.id;
   var content = commentField.value;
...
function exploreActivity(load = true) {
   ...
   for (let i in groupList) {
      switch (groupList[i].category) {
         ...
         default:
            pane=document.getElementById("other");
      }
...
function like(msgIndex){
   ...
   //  var query = "UPDATE posts SET likes = likes + 1 WHERE condition here ";
...
function dislike(msgIndex){
   ...
   //  var query = "UPDATE posts SET dislikes = dislikes + 1 WHERE some condition here ";
